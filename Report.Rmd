...
ggplot(vcf_with_metadata, aes(x = pos,y = qual)) +
  geom_point(size = 1.5) +
  labs(
...
ggplot(malaria_infection_only_long, aes(x = Status, y = Count, fill = Status)) +
  geom_bar(stat = "identity", position = "stack") +  # position = "stack" for stacking
  labs(
    title = "Malaria Infections vs Grand Total (Non-Malaria Infected)",
    x = "Category",
    y = "Count",
    fill = "Status"
  )
